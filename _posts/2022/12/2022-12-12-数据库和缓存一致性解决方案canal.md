...
mysql的主从复制原理：
 ...
 ### canal的安装配置（以windows为例）
   1、登进Mysql后，使用show variables like'log_bin';查询是否开启binlog，如果开启（ON），进行下一步，如果没开启(OFF)，在数据库的my.ini配置文件添加配置
  ...
  2、binlog开启后，创建一个canal用户并授权，官网配置是@%，表示所有服务器，所以改为localhost就可以，在mysql中，运行如下代码，设置完成之后重启：
   ...
   3、安装canal
   - 地址： https://github.com/alibaba/canal/releases/tag/canal-1.1.6-alpha-1
   ```
    canal.id = 1
    canal.ip =
    canal.port = 11111
   ...
   ```
   1. 说明：这个文件是 canal 的基本通用配置，canal 端口号默认就是 11111，修改 canal 的输出 model，默认 tcp，改为输出到 kafka  
   2. 重点关注上面的：canal.serverMode = tcp 这个配置，默认情况，如果是使用mysql，可以不做修改，如果需要将数据同步到kafka，或者rocketmq，可以分别修改即可，此处暂不做修改
  ...
  ```
   1. canal.instance.mysql.slaveId=20 #只要和mysql的master的不一样即可
   2. canal.instance.master.address=127.0.0.1:3306 ，监听的mysql的master节点信息
   3. 配置连接 MySQL 的用户名和密码，默认就是我们前面授权的 canal
...
![](http://www.javanorth.cn/assets/images/2022/coolhy/canal/2.png)
 ...
 最后我们拿到数据之后可以放入消息队列，这样可以加入重试机制，还可以防止幂等问题，最后再写入缓存。
