...
无论是已经成为Java精英界您，还是刚进入Java的新同学，应该都耳有所闻这个Java8引入的新特性。当然，对于指北君这个沉浸在Java世界的这只猿来说，Java8也是具有相当的年代感了，毕竟JDK11都快一统江湖了嘛，但依然不妨碍北北今天来给大家捯饬捯饬函数式接口这个熟悉的陌生人。

<!--more-->

## 概述

口水话说了一堆，指北君还是先对本次主题做个总括。
文章为JDK源码解析系列，本期的主题为Java8版本中引入的“函数式接口”，介于本次主题内容涉及较为广泛，文章将分为应用和原理两个篇章。读者可以根据自身的需求侧重阅读自己关注的内容。
应用篇将以java.util.function包为基础，围绕函数式接口的关键知识点进行介绍说明,将包含如下内容：FunctionInterface，Lambda表达式，双冒号操作符进行详细介绍和使用。
...
> (阅读导引，本节为概念性说明，技术向可跳过)

在分析具体内容之前，我们需要对函数式接口有个基本的认知。函数式接口是JAVA语言为引入函数式编程而增加的特性，我们暂时将目标转向函数式编程这个概念。那么，函数式编程到底是什么？他和面向对象编程又有什么关系？它能为我们带来什么？我们又是否真的需要函数式编程？
有很多小伙伴，可能和指北君一样，是以面向对象语言开启的编程世界，那么对于函数式编程其实很陌生。所以，指北君在这里先给大家引荐编程界的三大流派（当然还有别的流派）：过程式，函数式，对象式：
...
函数式编程的定义为："函数式编程是一种编程范式。它把计算当成是数学函数的求值，从而避免改变状态和使用可变数据。它是一种声明式的编程范式，通过表达式和声明而不是语句来编程。" 
...
上一节我们讲了什么是函数式编程，以及它有哪些优劣势，那么下面我们将介绍在Java中如何使用它。

可能大家有点疑惑，指北君为啥要从java.util.function开始呢？因为，在JDK中提供一些很常用的函数式接口，基本可以满足我们很多通用场景的使用，这就是java.util.function这个包。

![function包](/assets/images/2021/simsky/jdk_src_func_if_4.png)

打开源码一看，足足有43个接口，这43把剑摆在眼前让大家舞动，是不是感觉鸭梨山大了呢？别慌，在你看完指北君后面的分析后，你就可以将这43把剑化为无形，做到手中无剑心中有剑。
第一步：北北带领大家对接口名称进行分析，对接口名称，不是源码

![接口名称解析](/assets/images/2021/simsky/jdk_src_func_if_2.png)
接口的主体为最后的单词，包含Consumer，Function，Predicate，Supplier，Operator。
![接口主体类别](/assets/images/2021/simsky/jdk_src_func_if_3.png)

除了主体外我们还看到了Unary，Binary，Bi这种表示参数个数的修饰词。
...
通过以上解析，我们可以确定所有的接口都是用于描述函数的输入参数和返回，这就是java.util.function留在我们心中终极大剑。有了这把终极武器，我们可以对于这43把剑信手拈来，也随意创造自己的武器，当然，要创造新的按照这43把剑依葫芦画瓢也可以，但是要打造精品武器还需要掌握我们接下来介绍的FunctionInterface接口了。

在对整个java.util.function包了然于胸后，我们在打开一个典型的接口看看源码。function包中的类型都为接口类型，并且使用了@FunctionInterface注解，且每个接口都且只有一个接口（抽象）方法，部分接口存在默认方法和静态方法。
...
```
accept为核心接口方法，andThen为方便复杂组合场景提供的默认方法。function包的整个代码逻辑都简洁易懂，部分Operator继承了Function，指北君就不一一赘述了。

##FunctionInterface
上一节，指北君给小伙伴讲了java.util.function中的43把剑，并让大家做到心中有剑，这一节指北君将和小伙伴一同研究怎么造一柄自己的剑呢。
FunctoinInterface注解就是来帮我们铸造武器的工具，并且对我们的造剑质量把关。FunctionInterface用于显式声明接口为函数式接口，编译器会按照函数式接口的约束进行检查。
先看看注解的定义：
```java
@Documented
...
最后，还有一个重点知识：是不是只有使用了@FunctionalInterface才能作为函数式接口呢？相信有不少伙伴和指北君之前一样理所当然地觉得显然应该是这样嘛。但是，JDK的世界很精妙：只要符合函数式接口约束条件的接口，即使没有采用@FunctionalInterface，编译器都会处理成函数式接口。
```java
...
##Lambda表达式
听指北君讲了这么多，大家也做到了心中有剑和铸造自己的剑，这时候是不是想找个对手来练练呢？那就轮到我们的Lambda表达式登场了。
在渊源章节，指北君就说过函数式编程脱胎于λ演算，所以我们的战场名称也就叫Lambda。
下面是一段简单的Lambda代码示例：
```java
...
x -> System.out.print(x)就是Lambda表达式，从这里可以看出Lambda表达式是通过->云算法来连接的，左边为参数部分，右边为函数体。这里指北君为大家列出典型的集中Lambda表达式：
用法：
...
##双冒号操作符
在一节，指北君和小伙伴们学习了Lambda表达式，有些小伙伴可能会问，Java类中的的方法也是函数，我可不可以在传入Lambda表达式的地方传入方法呢？类似下面这种效果：
```java
    List<String> list = new ArrayList<String>();
    ...
    list.forEach(xxxMethod());
```
想法是支持的，但是形式错误了，xxxMethod()会直接触方法执行，并且返回的类型也不匹配forEach方法。那么，正确的形式应该如何写呢？现在就是我们的双冒号云算法登场了。
